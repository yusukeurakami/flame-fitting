...
def compute_approx_scale(lmk_3d, model, lmk_face_idx, lmk_b_coords, opt_options=None):
    ...
    ch.minimize(
        fun=lmk_err, x0=[scale, model.trans, model.pose[:3]], method='dogleg', callback=on_step, options=opt_options
    )
    return scale.r
...
def fit_scan(
    ...
    lmk_err = landmark_error_3d(
        mesh_verts=model, mesh_faces=model.f, lmk_3d=lmk_3d, lmk_face_idx=lmk_face_idx, lmk_b_coords=lmk_b_coords
    )
    ...
    ch.minimize(fun=lmk_err, x0=[model.trans, model.pose[:3]], method='dogleg', callback=on_step, options=opt_options)
    timer_end = time()
    ...
    ch.minimize(fun=objectives, x0=free_variables, method='dogleg', callback=on_step, options=opt_options)
    timer_end = time()
...
def run_fitting():
    ...
    scan_path = './data/facemodel/yusuke_01/yusuke_01_cleaned.obj'

    # landmarks of the scan
    # scan_lmk_path = './data/scan_lmks.npy'
    scan_lmk_path = './data/facemodel/yusuke_01/yusuke_01_cleaned.pp'
